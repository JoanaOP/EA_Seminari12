...
class ActivityRoutes {
  ...
  public async addActivity (req: Request, res: Response) : Promise<void> {
    ...
    if (user.active === false) {
      ...
      await User.findOneAndUpdate({ name: user.name }, { activitiesOrganized: user.activitiesOrganized })
    }
  ...
  public async addUserActivity (req: Request, res: Response) : Promise <void> {
    ...
    if (user == null || user.active === false) {
      ...
      await User.findOneAndUpdate({ _id: idUser }, { activities: user.activities })

      const activity = await Activity.findById(idActivity)
      activity.users.push(idUser)
      await Activity.findOneAndUpdate({ _id: idActivity }, { users: activity.users })
